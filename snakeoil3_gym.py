...
def drive_example(c, target_speed):
...
if __name__ == u"__main__":
    ...
    SAMPLE_NUM_EXPERT = 2000
    X_DIM_EXPERT = 29
    Y_DIM_EXPERT = 3
    data_obs_expert = np.zeros([SAMPLE_NUM_EXPERT, X_DIM_EXPERT])
    data_action_expert = np.zeros([SAMPLE_NUM_EXPERT, Y_DIM_EXPERT])

    SAMPLE_NUM_ROAD = 2000
    X_DIM_ROAD = 21
    data_obs_road = np.zeros([SAMPLE_NUM_ROAD, X_DIM_ROAD])
    ...
    for step in xrange(C.maxSteps,0,-1):
        ...
        if np.mod(step, TIMESTEP_LENGTH) == 0:
            target_speed = CRUISE_SPEED.function(TIMESTEP_LENGTH) * 100.0

        print("target speed:", target_speed)
        S, R = drive_example(C, target_speed)
        # print R[u'accel']
        # print R[u'brake']
        C.respond_to_server()

        # C2.get_servers_input()
        # S2, R2 = drive_example(C2)
        # C2.respond_to_server()
        #
        # print S[u'opponents']
        # print S[u'racePos']
        # print("^^^")
        # print S2[u'opponents']
        # print S2[u'racePos']

    # collect data for C1
        speedX=np.array(S[u'speedX'])/300.
        speedY=np.array(S[u'speedY'])/300.
        speedZ=np.array(S[u'speedZ'])/300.
        angle=np.array(S[u'angle'])/3.1416
        rpm=np.array(S[u'rpm'])/10000.
        track=np.array(S[u'track'])/200.
        trackPos=np.array(S[u'trackPos'])/1.
        wheelSpinVel=np.array(S[u'wheelSpinVel'])
        wheelSpinVel = np.array([e/100.0 for e in wheelSpinVel])

        accel = np.array(R[u'accel'])
        brake = np.array(R[u'brake'])
        steer = np.array(R[u'steer'])

        if EXPERT_DEMONSTRATION:
            obs_expert = np.hstack((angle, track, trackPos, speedX, speedY,  speedZ, wheelSpinVel, rpm))
            action_expert = np.hstack((steer, accel, brake))
            print(time_step)
            if time_step<SAMPLE_NUM_EXPERT:
                data_obs_expert[time_step, :] = obs_expert
                data_action_expert[time_step, :] = action_expert
            else:
                C.shutdown()
                data_obs_expert.tofile("expert_obs1.bin")
                data_action_expert.tofile("expert_action1.bin")
                print("finish collecting samples!")
            time_step += 1

        if ROAD_CLASSIFICATION:
            obs_road = np.hstack((angle, track, trackPos)) # 1+19+1=21 dims
            print(time_step)
            if time_step<SAMPLE_NUM_ROAD:
                data_obs_road[time_step, :] = obs_road
            else:
                C.shutdown()
                data_obs_road.tofile("classify_obs1.bin")
                print("finish collecting samples!")
            time_step += 1
        # Collect data for C1 -- END
